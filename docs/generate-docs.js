...
const children = [
      ...
      children: [
        new TableCell({
          children: [
            ...
            new Paragraph({ children: [new TextRun({ text: "Next.js · MongoDB · Stripe · fal.ai · Gemini AI", color: MID, size: 20 })], alignment: AlignmentType.CENTER, spacing: { before: 0, after: 60 } }),
          ],
  ...
  code("│   │   ├── stripe/checkout/      # POST — create Stripe Checkout session"),
  code("│   │   ├── stripe/portal/        # POST — Stripe billing portal"),
  code("│   │   ├── stripe/webhook/        # POST — Stripe payment confirmation"),
  ...
  code("│   ├── stripe.ts                 # Stripe client initialization"),
  ...
  numbered("User is redirected to /credits?success=true and sees updated balance", 6),
  ...
  p("Creates a Stripe Checkout session for purchasing a credit pack. Requires authentication."),

  code("POST /api/stripe/checkout"),
  code('Content-Type: application/json'),
  code('{ "plan": "starter" | "growth" | "pro" | "business" }'),
  code(""),
  code('Response: { "url": "<stripe_checkout_url>" }'),

  spacer(),
  h2("9.9  POST /api/stripe/webhook"),
  p("Handles Stripe checkout.session.completed events. Idempotent via session ID. Sends a receipt email via Resend."),

  code("POST /api/stripe/webhook"),
  code("Stripe-Signature: <header>"),
  code(""),
  code("// Internal: adds credits, sends receipt, marks session as processed"),

  spacer(),
  h2("9.10  POST /api/stripe/portal"),
  p("Creates a Stripe billing portal session for managing payment methods and invoices. Requires authentication."),

  code("POST /api/stripe/portal"),
  code('Response: { "url": "<stripe_portal_url>" }'),
  ...
  p("To test Stripe webhooks locally, use the Stripe CLI:"),
  code("stripe listen --forward-to localhost:3000/api/stripe/webhook"),
  code("# Copy the webhook signing secret (whsec_...) output by this command"),
  code("# Paste it as STRIPE_WEBHOOK_SECRET in .env.local"),
